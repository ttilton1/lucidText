 ...
 class HomeViewController: UIViewController {
    ...
    DispatchQueue(
     ...
     // DECODE THE RESULTING JSON
      ...
      func analyzeWords(thresh: Int, completion: @escaping (_ difficulWords: Set<String>?) -> ()) {
         //text: Set<String>, thresh: Int, completion: @escaping (_ difficulWords: Set<String>?) -> ()) {
       //  concurrentPhotoQueue.async(flags: .barrier) { [weak self] in
         
      //   let q = DispatchQueue(label: "com.theswiftdev.queues.serial")
      //   q.sync {
 /* Here it starts
         for phrase in newHardWordsCheck {
             self.getWordDifficulty(word: phrase) {  matchScore in
              ...
              if (freq > thresh){
                  print("i'm here")
                  newHardWordsCheck.remove(phrase)
                  print("print:\(newHardWordsCheck)")
              }
             }
              
             }
    Here it ends     */
       //  completion(newHardWordsCheck)
     ...
     @IBAction func processTouched(_ sender: Any) {
         ...
         let separators = CharacterSet(charactersIn: " .,:!?;/[]-)(\n")
          var parts = inputString.components(separatedBy: separators)
